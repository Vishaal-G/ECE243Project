...
#define SIREN_HALF_PERIOD_FRAMES 30
#define SIREN_FREQ_HI 1200
#define SIREN_FREQ_LO  800
#define SIREN_AMPLITUDE 0x06000000
...
/* ── siren state ─────────────────────────────────────────────────── */
static int  siren_phase_accum    = 0;
static int  siren_half_frame_cnt = 0;
static bool siren_hi_tone        = true;
/* ──────────────────────────────────────────────────────────────────── */
...
void update_siren(void) {
  ...
  if (!playing) {
    ...
    free_slots  = fifo_status & 0xFF;
    if (free_slots > 16) { free_slots = 16; }
...
void generate_map(void) {
  ...
  for (branch = 0; branch < 14; branch++) {
    ...
    if (!is_road_tile(branch_col, branch_row)) { continue; }
    if ((rand_u32() & 1u) == 0u) {
      for (col = 0; col < length; col++) {
        carve_col = branch_col + dir * col;
        if (carve_col <= 0 || carve_col >= MAP_COLS - 1) { break; }
        world_map[branch_row][carve_col] = TILE_ROAD;
      }
    } else {
      for (row = 0; row < length; row++) {
        carve_row = branch_row + dir * row;
        if (carve_row <= 0 || carve_row >= MAP_ROWS - 1) { break; }
...
void place_buildings(void) {
  ...
  for (row = 2; row < MAP_ROWS - 3; row++) {
    for (col = 2; col < MAP_COLS - 3; col++) {
      if (world_map[row][col] != TILE_GRASS) { continue; }
      if ((rand_u32() % 100u) >= 48u) { continue; }
      ...
      for (r = row; r < row + block_h; r++) {
        for (c = col; c < col + block_w; c++) {
          if (world_map[r][c] != TILE_GRASS) { can_place = false; }
        }
      }
    ...
    for (row = 1; row < MAP_ROWS - 1; row++) {
      for (col = 1; col < MAP_COLS - 1; col++) {
        ...
        if (touches_road && left_blocked && right_blocked && vertical_corridor) {
          if (world_map[row][col - 1] == TILE_BUILDING) { world_map[row][col - 1] = TILE_GRASS; }
          if (world_map[row][col + 1] == TILE_BUILDING) { world_map[row][col + 1] = TILE_GRASS; }
        }
        if (touches_road && up_blocked && down_blocked && horizontal_corridor) {
          if (world_map[row - 1][col] == TILE_BUILDING) { world_map[row - 1][col] = TILE_GRASS; }
          if (world_map[row + 1][col] == TILE_BUILDING) { world_map[row + 1][col] = TILE_GRASS; }
        }
      }
    }
...
bool pickup_can_spawn_at(int col, int row) {
  ...
  if (!is_road_tile(col, row)) { return false; }
  if (row <= 3 && col <= 3) { return false; }

  for (check_row = row - 1; check_row <= row + 1; check_row++) {
    for (check_col = col - 1; check_col <= col + 1; check_col++) {
      if (check_row < 0 || check_col < 0 || check_row >= MAP_ROWS || check_col >= MAP_COLS) { continue; }
      if (cash_pickups[check_row][check_col]) { return false; }
      if (speed_boost_pickups[check_row][check_col]) { return false; }
    }
  }
  return true;
}
...
bool speed_boost_can_spawn_at(int col, int row) {
  ...
  for (check_row = row - 2; check_row <= row + 2; check_row++) {
    for (check_col = col - 2; check_col <= col + 2; check_col++) {
      if (check_row < 0 || check_col < 0 || check_row >= MAP_ROWS || check_col >= MAP_COLS) { continue; }
      if (cash_pickups[check_row][check_col] || speed_boost_pickups[check_row][check_col]) { return false; }
    }
  }
  return true;
}
...
int get_target_police_count(void) {
  int count = 1 + score / 5;
  if (count > MAX_POLICE_CARS) { count = MAX_POLICE_CARS; }
  return count;
}
...
void spawn_police_car_at_index(int index) {
  ...
  for (attempts = 0; attempts < 256; attempts++) {
    ...
    if (!is_road_tile(col, row)) { continue; }
    spawn_x = (col + 0.5f) * TILE_SIZE;
    spawn_y = (row + 0.5f) * TILE_SIZE;
    dx = spawn_x - player.x;
    dy = spawn_y - player.y;
    if (dx * dx + dy * dy < 170.0f * 170.0f) { continue; }
...
void sync_police_cars_to_score(void) {
  ...
  int target_count = get_target_police_count();
  for (i = 0; i < target_count; i++) {
    if (!police_cars[i].active) { spawn_police_car_at_index(i); }
  }
...
void handle_keyboard_byte(unsigned char byte) {
  if (byte == 0xE0) { extended_code = true; return; }
  if (byte == 0xF0) { break_code = true; return; }
  if (!extended_code) { update_key_state(byte, !break_code); }
  break_code = false;
  extended_code = false;
}
...
void update_key_state(unsigned char scan, bool pressed) {
  switch (scan) {
    case 0x1D: key_w = pressed; break;
    case 0x1B: key_s = pressed; break;
    case 0x1C: key_a = pressed; break;
    case 0x23: key_d = pressed; break;
    case 0x2D: key_r = pressed; break;
    default: break;
  }
}
...
TileType get_tile_at_world(float world_x, float world_y) {
  ...
  if (col < 0 || row < 0 || col >= MAP_COLS || row >= MAP_ROWS) { return TILE_BORDER; }
...
bool check_collision(float next_x, float next_y) {
  ...
  for (row = min_row; row <= max_row; row++) {
    for (col = min_col; col <= max_col; col++) {
      if (is_blocking_tile(col, row)) { return true; }
    }
  }
  return false;
}
...
bool has_line_of_sight(float start_x, float start_y, float end_x, float end_y) {
  ...
  for (step = 1; step < steps; step++) {
    ...
    if (is_blocking_tile((int)(sample_x / TILE_SIZE), (int)(sample_y / TILE_SIZE))) {
      return false;
    }
  }
  return true;
...
/* OPTIMIZATION: BFS chase map only runs every 3 frames */
void update_chase_distance_map(void) {
  ...
  if (is_blocking_tile(start_col, start_row)) { return; }
  ...
  while (head < tail && tail <= max_tiles) {
    ...
    for (dir = 0; dir < 4; dir++) {
      ...
      if (next_col < 0 || next_row < 0 || next_col >= MAP_COLS || next_row >= MAP_ROWS) { continue; }
      if (is_blocking_tile(next_col, next_row) || chase_distance_map[next_row][next_col] >= 0) { continue; }
...
void ensure_police_car_valid(int index) {
  if (!police_cars[index].active) { return; }
...
bool check_player_police_collision_for_index(int index) {
  ...
  if (!police_car->active) { return false; }
  ...
  if (distance_sq >= min_distance * min_distance) { return false; }

  distance = sqrtf(distance_sq);
  if (distance < 0.001f) {
    nx = 1.0f; ny = 0.0f; overlap = min_distance;
  } else {
    nx = dx / distance; ny = dy / distance; overlap = min_distance - distance;
  }
  ...
  if (check_collision(player.x, player.y)) {
    player.x -= nx * overlap; player.y -= ny * overlap; player.speed = 0.0f;
  }
  if (check_collision(police_car->x, police_car->y)) {
    police_car->x += nx * overlap; police_car->y += ny * overlap; police_car->speed = 0.0f;
  }
  ...
  if (distance_sq < min_distance * min_distance && distance_sq > 0.001f) {
    distance = sqrtf(distance_sq);
    nx = dx / distance; ny = dy / distance; overlap = min_distance - distance;
    player.x += nx * (overlap * 0.5f); player.y += ny * (overlap * 0.5f);
    police_car->x -= nx * (overlap * 0.5f); police_car->y -= ny * (overlap * 0.5f);
  }

  if (police_freeze_frames <= 0) { register_player_hit_for_index(index); }
  return true;
}
...
bool check_player_police_collision(void) {
  ...
  bool collided = false;
  for (i = 0; i < MAX_POLICE_CARS; i++) {
    if (check_player_police_collision_for_index(i)) { collided = true; }
  }
  return collided;
}
...
void register_player_hit_for_index(int index) {
  ...
  if (police_freeze_frames > 0 || player_lives <= 0) { return; }
  ...
  if (distance < 0.001f) { nx = 1.0f; ny = 0.0f; }
  ...
  if (check_collision(police_car->x, police_car->y)) {
    ...
    for (attempt = 2; attempt <= 5 && !found_spot; attempt++) {
      ...
      for (side = -1; side <= 1; side++) {
        candidate_x = police_car->x + nx * safe_distance + side_x * (float)side * POLICE_PLAYER_HIT_RADIUS;
        candidate_y = police_car->y + ny * safe_distance + side_y * (float)side * POLICE_PLAYER_HIT_RADIUS;
        if (!check_collision(candidate_x, candidate_y)) {
          police_car->x = candidate_x; police_car->y = candidate_y;
          found_spot = true; break;
        }
      }
    }
    if (!found_spot) { spawn_police_car_at_index(index); }
  }
...
void update_timers(void) {
  if (police_freeze_frames > 0) {
    ...
    if (police_freeze_frames == 0) {
      int i;
      for (i = 0; i < MAX_POLICE_CARS; i++) {
        if (police_cars[i].active) { police_cars[i].speed = 1.2f; }
      }
    }
  }
  if (speed_boost_frames > 0) { speed_boost_frames--; }
}
...
void collect_cash_pickups(void) {
  ...
  for (row = min_row; row <= max_row; row++) {
    for (col = min_col; col <= max_col; col++) {
      ...
      if (!cash_pickups[row][col] && !speed_boost_pickups[row][col]) { continue; }
...
void update_player(void) {
  ...
  if (key_a) { player.angle += turn_rate * (0.4f + fabsf(player.speed)); }
  if (key_d) { player.angle -= turn_rate * (0.4f + fabsf(player.speed)); }

  if (player.angle > 2.0f * PI) { player.angle -= 2.0f * PI; }
  else if (player.angle < 0.0f) { player.angle += 2.0f * PI; }

  if (key_w) { player.speed += current_accel_forward; }
  if (key_s) {
  ...
  if (!key_w && !key_s) { player.speed *= idle_drag; }

  player.speed *= traction;
  player.speed = clamp_float(player.speed, max_reverse_speed, current_max_forward_speed);
  ...
  if (!check_player_collision(next_x, player.y)) { player.x = next_x; }
  else { player.speed = old_speed * -0.2f; }

  if (!check_player_collision(player.x, next_y)) { player.y = next_y; }
  else { player.speed = old_speed * -0.2f; }

  if (check_player_collision(player.x, player.y)) {
    player.x = old_x; player.y = old_y; player.speed = 0.0f;
  }
}
...
void update_single_police_car(int index) {
  ...
  if (!police_car->active) { return; }
  if (police_freeze_frames > 0) { police_car->speed = 0.0f; return; }

  get_police_difficulty(&chase_accel, &chase_drag, &chase_max_speed, &chase_turn_rate, &lead_scale);

  police_target_x[index] = player.x;
  police_target_y[index] = player.y;

  if (has_line_of_sight(police_car->x, police_car->y, player.x, player.y)) {
    police_target_x[index] = player.x + cosf(player.angle) * player.speed * lead_scale;
    police_target_y[index] = player.y - sinf(player.angle) * player.speed * lead_scale;
  } else {
    ...
    for (step_row = -1; step_row <= 1; step_row++) {
      for (step_col = -1; step_col <= 1; step_col++) {
        ...
        if ((step_col == 0 && step_row == 0) || (step_col != 0 && step_row != 0)) { continue; }
        if (next_col < 0 || next_row < 0 || next_col >= MAP_COLS ||
            next_row >= MAP_ROWS || is_blocking_tile(next_col, next_row)) { continue; }

        next_distance = chase_distance_map[next_row][next_col];
        if (next_distance < 0) { continue; }
        if (best_distance < 0 || next_distance < best_distance) {
          best_distance = next_distance; best_col = next_col; best_row = next_row;
        }
      }
    }
  ...
  target_x = clamp_float(police_target_x[index], 1.5f * TILE_SIZE, (MAP_COLS - 1.5f) * TILE_SIZE);
  target_y = clamp_float(police_target_y[index], 1.5f * TILE_SIZE, (MAP_ROWS - 1.5f) * TILE_SIZE);
  ...
  if (angle_diff > chase_turn_rate)       { angle_diff = chase_turn_rate; }
  else if (angle_diff < -chase_turn_rate) { angle_diff = -chase_turn_rate; }

  police_car->angle += angle_diff;
  if (police_car->angle > 2.0f * PI) { police_car->angle -= 2.0f * PI; }
  else if (police_car->angle < 0.0f) { police_car->angle += 2.0f * PI; }

  police_car->speed += chase_accel;
  if (police_car->speed > chase_max_speed) { police_car->speed = chase_max_speed; }
  ...
  if (!check_collision(next_x, police_car->y)) { police_car->x = next_x; }
  else {
  ...
  if (!check_collision(police_car->x, next_y)) { police_car->y = next_y; }
  else {
...
/* OPTIMIZATION: BFS runs every 3 frames only */
void update_police_car(void) {
  static int chase_update_counter = 0;
  int i;

  sync_police_cars_to_score();

  chase_update_counter++;
  if (chase_update_counter >= 3) {
    chase_update_counter = 0;
...
void update_score_display(void) {
  ...
  int digit1 = (value > 0) ? (value % 10) : -1; value /= 10;
  int digit2 = (value > 0) ? (value % 10) : -1; value /= 10;
  int digit3 = (value > 0) ? (value % 10) : -1; value /= 10;
  int digit4 = (value > 0) ? (value % 10) : -1; value /= 10;
  int digit5 = (value > 0) ? (value % 10) : -1;
  int hex30 = (int)encode_hex_digit(digit0) | ((int)encode_hex_digit(digit1) << 8) |
              ((int)encode_hex_digit(digit2) << 16) | ((int)encode_hex_digit(digit3) << 24);
  int hex54 = (int)encode_hex_digit(digit4) | ((int)encode_hex_digit(digit5) << 8);
  *HEX3_HEX0_ptr = hex30;
  *HEX5_HEX4_ptr = hex54;
}
...
void update_lives_display(void) {
  ...
  if (player_lives <= 0)          { led_mask = 0u; }
  else if (player_lives >= MAX_LIVES) { led_mask = (1u << MAX_LIVES) - 1u; }
  else                            { led_mask = (1u << player_lives) - 1u; }
...
void draw_world(void) {
  ...
  for (row = start_row; row <= end_row; row++) {
    for (col = start_col; col <= end_col; col++) {
      if (row < 0 || col < 0 || row >= MAP_ROWS || col >= MAP_COLS) { continue; }
...
unsigned char get_glyph_row(char ch, int row) {
  switch (ch) {
    case 'A': { static const unsigned char r[7] = {0x0E,0x11,0x11,0x1F,0x11,0x11,0x11}; return r[row]; }
    case 'B': { static const unsigned char r[7] = {0x1E,0x11,0x11,0x1E,0x11,0x11,0x1E}; return r[row]; }
    case 'C': { static const unsigned char r[7] = {0x0E,0x11,0x10,0x10,0x10,0x11,0x0E}; return r[row]; }
    case 'D': { static const unsigned char r[7] = {0x1E,0x11,0x11,0x11,0x11,0x11,0x1E}; return r[row]; }
    case 'E': { static const unsigned char r[7] = {0x1F,0x10,0x10,0x1E,0x10,0x10,0x1F}; return r[row]; }
    case 'F': { static const unsigned char r[7] = {0x1F,0x10,0x10,0x1E,0x10,0x10,0x10}; return r[row]; }
    case 'G': { static const unsigned char r[7] = {0x0E,0x11,0x10,0x17,0x11,0x11,0x0E}; return r[row]; }
    case 'H': { static const unsigned char r[7] = {0x11,0x11,0x11,0x1F,0x11,0x11,0x11}; return r[row]; }
    case 'I': { static const unsigned char r[7] = {0x0E,0x04,0x04,0x04,0x04,0x04,0x0E}; return r[row]; }
    case 'L': { static const unsigned char r[7] = {0x10,0x10,0x10,0x10,0x10,0x10,0x1F}; return r[row]; }
    case 'M': { static const unsigned char r[7] = {0x11,0x1B,0x15,0x15,0x11,0x11,0x11}; return r[row]; }
    case 'N': { static const unsigned char r[7] = {0x11,0x19,0x15,0x13,0x11,0x11,0x11}; return r[row]; }
    case 'O': { static const unsigned char r[7] = {0x0E,0x11,0x11,0x11,0x11,0x11,0x0E}; return r[row]; }
    case 'P': { static const unsigned char r[7] = {0x1E,0x11,0x11,0x1E,0x10,0x10,0x10}; return r[row]; }
    case 'R': { static const unsigned char r[7] = {0x1E,0x11,0x11,0x1E,0x14,0x12,0x11}; return r[row]; }
    case 'S': { static const unsigned char r[7] = {0x0F,0x10,0x10,0x0E,0x01,0x01,0x1E}; return r[row]; }
    case 'T': { static const unsigned char r[7] = {0x1F,0x04,0x04,0x04,0x04,0x04,0x04}; return r[row]; }
    case 'U': { static const unsigned char r[7] = {0x11,0x11,0x11,0x11,0x11,0x11,0x0E}; return r[row]; }
    case 'V': { static const unsigned char r[7] = {0x11,0x11,0x11,0x11,0x11,0x0A,0x04}; return r[row]; }
    case ':': { static const unsigned char r[7] = {0x00,0x04,0x04,0x00,0x04,0x04,0x00}; return r[row]; }
    case '0': { static const unsigned char r[7] = {0x0E,0x11,0x13,0x15,0x19,0x11,0x0E}; return r[row]; }
    case '1': { static const unsigned char r[7] = {0x04,0x0C,0x04,0x04,0x04,0x04,0x0E}; return r[row]; }
    case '2': { static const unsigned char r[7] = {0x0E,0x11,0x01,0x02,0x04,0x08,0x1F}; return r[row]; }
    case '3': { static const unsigned char r[7] = {0x1E,0x01,0x01,0x0E,0x01,0x01,0x1E}; return r[row]; }
    case '4': { static const unsigned char r[7] = {0x02,0x06,0x0A,0x12,0x1F,0x02,0x02}; return r[row]; }
    case '5': { static const unsigned char r[7] = {0x1F,0x10,0x10,0x1E,0x01,0x01,0x1E}; return r[row]; }
    case '6': { static const unsigned char r[7] = {0x0E,0x10,0x10,0x1E,0x11,0x11,0x0E}; return r[row]; }
    case '7': { static const unsigned char r[7] = {0x1F,0x01,0x02,0x04,0x08,0x08,0x08}; return r[row]; }
    case '8': { static const unsigned char r[7] = {0x0E,0x11,0x11,0x0E,0x11,0x11,0x0E}; return r[row]; }
    case '9': { static const unsigned char r[7] = {0x0E,0x11,0x11,0x0F,0x01,0x01,0x0E}; return r[row]; }
    default: return 0x00;
  }
}
...
void draw_text(int x, int y, const char* text, int scale, short int color) {
  ...
  for (index = 0; text[index] != '\0'; index++) {
    if (text[index] == ' ') { cursor_x += 6 * scale; continue; }
    for (r = 0; r < 7; r++) {
      unsigned char bits = get_glyph_row(text[index], r);
      for (c = 0; c < 5; c++) {
        if ((bits & (1u << (4 - c))) != 0u) {
          draw_rect(cursor_x + c * scale, y + r * scale, scale, scale, color);
        }
      }
    }
    cursor_x += 6 * scale;
...
int get_text_width(const char* text, int scale) {
  int width = 0;
  int index;
  for (index = 0; text[index] != '\0'; index++) { width += 6 * scale; }
...
void draw_game_over_screen(void) {
  ...
  const int gap = 10;
  const int content_height = title_height + body_height * 4 + restart_height + gap * 5;
  ...
  if (score == 0) { score_digits[score_length++] = '0'; }
  else {
    int value = score;
    while (value > 0 && score_length < 11) { score_digits[score_length++] = (char)('0' + (value % 10)); value /= 10; }
  }
  ...
  if (best_score == 0) { best_digits[best_length++] = '0'; }
  else {
    int value = best_score;
    while (value > 0 && best_length < 11) { best_digits[best_length++] = (char)('0' + (value % 10)); value /= 10; }
  }
  ...
  clear_screen(BLACK);
  draw_text((SCREEN_W - get_text_width(title,         title_scale))   / 2, start_y,                                          title,         title_scale,   RED);
  draw_text((SCREEN_W - get_text_width(current_label, body_scale))    / 2, start_y + title_height + gap,                     current_label, body_scale,    WHITE);
  draw_text((SCREEN_W - get_text_width(score_digits,  body_scale))    / 2, start_y + title_height + body_height + gap * 2,   score_digits,  body_scale,    YELLOW);
  draw_text((SCREEN_W - get_text_width(best_label,    body_scale))    / 2, start_y + title_height + body_height * 2 + gap * 3, best_label,  body_scale,    WHITE);
  draw_text((SCREEN_W - get_text_width(best_digits,   body_scale))    / 2, start_y + title_height + body_height * 3 + gap * 4, best_digits, body_scale,    GREEN);
  draw_text((SCREEN_W - get_text_width(restart_label, restart_scale)) / 2, start_y + title_height + body_height * 4 + gap * 5, restart_label, restart_scale, LIGHT_GRAY);
}
...
void draw_health_bar(void) {
  ...
  if (player_lives <= 2)           { fill_color = RED; }
  else if (player_lives < MAX_LIVES) { fill_color = YELLOW; }
  if (police_freeze_frames > 0)    { border_color = BLUE; }
  ...
  draw_rect(boost_panel_x, boost_panel_y + boost_panel_h - 1, boost_panel_w, 1, boost_color);
  draw_rect(boost_panel_x, boost_panel_y, 1, boost_panel_h, boost_color);
  draw_rect(boost_panel_x + boost_panel_w - 1, boost_panel_y, 1, boost_panel_h, boost_color);
...
void draw_tile(int col, int row, int screen_x, int screen_y) {
  ...
  if      (tile == TILE_GRASS)    { fill = DARK_GREEN; }
  else if (tile == TILE_ROAD)     { fill = GRAY; }
  else if (tile == TILE_BUILDING) { fill = BROWN; }
  else if (tile == TILE_BORDER)   { fill = BLUE; }

  draw_rect(screen_x, screen_y, TILE_SIZE, TILE_SIZE, fill);

  if (tile == TILE_BUILDING) {
    draw_rect(screen_x + 4, screen_y + 4, TILE_SIZE - 8, TILE_SIZE - 8, DARK_GRAY);
...
/* OPTIMIZATION: replaced per-pixel loop with draw_rect for much faster rendering */
void draw_filled_car(float world_x, float world_y, float angle,
                     short int body_color, short int nose_color) {
  ...
  /* Draw body as axis-aligned rect — much faster than per-pixel rotation */
  draw_rect(cx - half_width, cy - half_length,
            (int)CAR_WIDTH, (int)CAR_LENGTH, body_color);

  /* Nose line shows direction */
  draw_line(cx, cy,
            cx + (int)(forward_x * (CAR_LENGTH * 0.7f)),
            cy + (int)(forward_y * (CAR_LENGTH * 0.7f)),
            nose_color);
}
